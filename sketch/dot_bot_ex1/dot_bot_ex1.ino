...
uint8_t M2_REEL_IN  = FORWARD;
uint8_t M2_REEL_OUT = BACKWARD;
...
float oldyy = 0;
String temp;
char standbyMess[55] = "Pick a plant to discover its route > > > > > > > > >";
int plant_index;
// motor position
...
static void FindHome() {
  ...
  laststep1 = safe_out + 1600; // cilynder offsets
  ...
  laststep2 = safe_out + 1600; // cilynder offsets
...
void reel(int index) {
  ...
  if (index == 10)
    m1->step(100, M1_REEL_IN, SINGLE );
  else if ( index == 11)
    m1->step(100, M1_REEL_OUT, SINGLE );
  else if (index == 20)
    m2->step(100, M2_REEL_IN, SINGLE );
  else
    m2->step(100, M2_REEL_OUT, SINGLE );
}
