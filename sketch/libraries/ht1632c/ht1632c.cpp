...
/* scrolling two texts functions */

void ht1632c::hscrolltexts(int y1, char *text1, uint8_t color1, int delaytime, int y2, char *text2, uint8_t color2,
                           int times, uint8_t dir, uint8_t attr1, uint8_t bgcolor1, uint8_t attr2, uint8_t bgcolor2)
{
  uint8_t showcolor1, showcolor2;
  int i, x, offs, len1 = strlen(text1);
  ...
  while (times) {
    for ((dir) ? x = - (len * width) : x = x_max; (dir) ? x <= x_max : x > - (len * width); (dir) ? x++ : x--)
    {
      for (i = 0; i < len1; i++)
      {
        offs = width * i;
        (dir) ? offs-- : offs++;
        putchar(x + offs,  y1, text1[i], bgcolor1, attr1, bgcolor1);
      }
      for (i = 0; i < len2; i++)
      {
        offs = width * i;
        (dir) ? offs-- : offs++;
        putchar(x + offs,  y2, text2[i], bgcolor2, attr2, bgcolor2);
      }
      for (i = 0; i < len1; i++)
      {
        showcolor1 = (color1 & RANDOMCOLOR) ? _rnd(1, 4) : color1;
        showcolor1 = ((color1 & BLINK) && (x & 2)) ? BLACK : (showcolor1 & ~BLINK);
        offs = width * i;
        putchar(x + offs,  y1, text1[i], showcolor1, attr1, bgcolor1);
      }
      for (i = 0; i < len2; i++)
      {
        showcolor2 = (color2 & RANDOMCOLOR) ? _rnd(1, 4) : color2;
        showcolor2 = ((color2 & BLINK) && (x & 2)) ? BLACK : (showcolor2 & ~BLINK);
        offs = width * i;
        putchar(x + offs,  y2, text2[i], showcolor2, attr2, bgcolor2);
      }
      sendframe();
      delay(delaytime);
    }
...
void ht1632c::puttext(int x, int y, char *text, uint8_t color, uint8_t bgcolor){
  uint8_t showcolor;
  int i, offs, len = strlen(text);
  uint8_t width = font_width;
  uint8_t height = font_height;
  width++;
  ...
  for (i = 0; i < len; i++)
  {
      offs = width * i;
      //(dir) ? offs-- : offs++;
      putchar(x + offs + 3,  y, text[i], bgcolor, 0, bgcolor);
  }
  for (i = 0; i < len; i++)
  {
     ...
     offs = width * i;
     putchar(x + offs + 3,  y, text[i], showcolor, 0, bgcolor);
  }
  sendframe();
}
